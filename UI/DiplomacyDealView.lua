...
local ms_IconAndTextLeftAndRightIM			:table		= InstanceManager:new( "IconAndTextLeftAndRight",  "SelectButton", Controls.IconAndTextContainer );
...
function FindIconInstanceFromControl(rootControl : table)
	if (rootControl ~= nil) then
		...
		controlTable = ms_IconAndTextLeftAndRightIM:FindInstanceByControl(rootControl);
		if (controlTable ~= nil) then
			return controlTable;
		end
...
-- ===========================================================================
function OnClickAvailableGreatWork(player, type, greatWorkIndex)
	...
	if (pDeal ~= nil) then
		...
		if (pDealItem == nil) then 
			...
			if (pDealItem ~= nil) then
				pDealItem:SetValueType(type);
				pDealItem:SetSubType(greatWorkIndex); -- using this for index of the great work in the GreatWorks table
				-- pDealItem:SetParameter('artifactOwner');
				-- pDealItem:SetParameterValue('artifactOwner', 45);
				-- print('sss ' .. pDealItem:GetParameterValue('artifactOwner'));
				--pDealItem:SetAmount(type); -- using this for artifact owner value (player id)
	...
	if (possibleResources ~= nil) then
		for i, entry in ipairs(possibleResources) do
			if (entry.Duration == 0) then
				...
				icon.SelectButton:SetToolTipString("Gold per turn");		-- We recycle the entries, so make sure this is clear.
	...
	function getImportedResources(playerID)
		...
		for _, pOtherPlayer in ipairs(kPlayers) do
			...
			if ( otherID ~= playerID ) then
				...
				local pDeals        :table = DealManager.GetPlayerDeals(playerID, otherID, DealItemTypes.RESOURCES);
							...
							if ( pDealResources ~= nil ) then
								for j,pDealResource in ipairs(pDealResources) do
									...
									local convertedResources = {
										...
										ImportString = "[COLOR_Gold]Trade[ENDCOLOR] with " .. Locale.Lookup(PlayerConfigurations[pOtherPlayer:GetID()]:GetPlayerName());
									};
				...
				if suzerainID == playerID then
					for row in GameInfo.Resources() do
						...
						if resourceAmount > 0 then
							...
							table.insert(importedResources, {
	...
	if (possibleItems ~= nil) then
		...
		for i, entry in ipairs(possibleItems) do
			...
			if (greatWorkDesc ~= nil) then
				...
				local type = entry.ForType;
				local icon = renderGreatWork(greatWorkDesc, type, player, iconList);

				icon.SelectButton:SetDisabled( not entry.IsValid and entry.ValidationResult ~= DealValidationResult.MISSING_DEPENDENCY );	-- Hide if invalid, unless it is just missing a dependency, the user will update that when it is added to the deal.
	
				-- What to do when double clicked/tapped.
				icon.SelectButton:RegisterCallback( Mouse.eLClick, function() OnClickAvailableGreatWork(player, type, entry.ForTypeDescriptionID); end );
...
function PopulateDealGreatWorks(player : table, iconList : table)
	...
	if (pDeal ~= nil) then 
		ms_IconAndTextIM:ReleaseInstanceByParent(iconList.ListStack);
		ms_IconAndTextLeftAndRightIM:ReleaseInstanceByParent(iconList.ListStack);
		...
		for pDealItem in pDeal:Items() do
			...
			if (pDealItem:GetFromPlayerID() == player:GetID()) then
				...
				if (type == DealItemTypes.GREATWORK) then
					local creatorType = pDealItem:GetValueType();
					local greatWork =  GameInfo.GreatWorks[pDealItem:GetSubType()];
					local icon = renderGreatWork(greatWork, creatorType, player, iconList);									
	...
	-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function renderGreatWork(greatWork, creatorType, player : table, iconList : table)
		--local greatWork = GameInfo.GreatWorks[entry.ForTypeDescriptionID];
		local icon = ms_IconAndTextLeftAndRightIM:GetInstance(iconList.ListStack);
		local gwText = '';
		local tooltipText = '';
		icon.Container_CivIcon:SetHide(true);

		-- for r,s in pairs(greatWork) do
		-- 		print(r,s);
		-- end
		-- print('-----------------');
						
		if greatWork.GreatWorkObjectType == 'GREATWORKOBJECT_ARTIFACT' then
			local artifactPlayer = Game.GetGreatWorkPlayer(creatorType);
			--local artifactPlayer = Game.GetGreatWorkPlayer(2);
			local primaryColor, secondaryColor = UI.GetPlayerColors( artifactPlayer );
			icon.Container_CivIcon:SetHide(false);
			icon.CivIcon:SetColor(secondaryColor);
			icon.CivIconBacking:SetColor(primaryColor);

			gwText = Locale.Lookup(greatWork.Name) .. '[NEWLINE][COLOR_Culture]' .. Locale.Lookup('LOC_' .. greatWork.EraType .. '_NAME') .. '[ENDCOLOR]';
			SetSpecifiedIconToSize(icon.CivIcon, 'ICON_' .. PlayerConfigurations[artifactPlayer]:GetCivilizationTypeName(), 34);
			SetSpecifiedIconToSize(icon.TypeIcon, 'ICON_' .. greatWork.GreatWorkObjectType .. '_ERA_ANCIENT', 40);
			tooltipText = Locale.Lookup(greatWork.Name) .. '[NEWLINE]' .. Locale.Lookup('LOC_' .. greatWork.GreatWorkObjectType .. '_' .. greatWork.EraType);
			tooltipText = tooltipText .. '[NEWLINE]Created by: ' .. Locale.Lookup('LOC_' .. PlayerConfigurations[artifactPlayer]:GetCivilizationTypeName() .. '_DESCRIPTION');

		elseif greatWork.GreatWorkObjectType == 'GREATWORKOBJECT_RELIC' then
			gwText = Locale.Lookup(greatWork.Name);
			SetSpecifiedIconToSize(icon.TypeIcon, 'ICON_' .. greatWork.GreatWorkObjectType, 40);
			tooltipText = Locale.Lookup(greatWork.Name) .. '[NEWLINE]' .. Locale.Lookup('LOC_' .. greatWork.GreatWorkObjectType);
		else
			gwText = Locale.Lookup(greatWork.Name) .. '[NEWLINE][COLOR_GoldMetal]' .. Locale.Lookup("LOC_" .. greatWork.GreatPersonIndividualType .. '_NAME') .. '[ENDCOLOR]';
			SetSpecifiedIconToSize(icon.TypeIcon, 'ICON_' .. greatWork.GreatWorkObjectType, 40);
			tooltipText = Locale.Lookup(greatWork.Name) .. '[NEWLINE]'.. Locale.Lookup('LOC_' .. greatWork.GreatWorkObjectType);
			tooltipText = tooltipText .. '[NEWLINE]Created by: ' .. Locale.Lookup("LOC_" .. greatWork.GreatPersonIndividualType .. '_NAME');
		end


		SetIconToSize(icon, "ICON_" .. greatWork.GreatWorkType, 42);
		icon.IconText:SetText(gwText);
		-- Set a tool tip
...
function OnShow()
	...
	ms_IconAndTextLeftAndRightIM:ResetInstances();
